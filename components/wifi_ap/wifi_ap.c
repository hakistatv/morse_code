/*
 * SoftAP bring-up, carried over from ../qr_code_wallet's wifi_ap.c. The
 * SSID/password come from device_settings (current_ssid/current_pass),
 * which loads them from NVS with the device_config.h values as the
 * first-boot defaults -- both are editable from the /settings web page.
 */
#include <string.h>
#include "esp_wifi.h"
#include "esp_event.h"
